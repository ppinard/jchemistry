...
public final class Generator {
    ...
    public AtomSite apply(AtomSite atom) {
        double px = atom.getPosition().getX();
        double py = atom.getPosition().getY();
        double pz = atom.getPosition().getZ();

        double x = m[0][0] * px + m[0][1] * py + m[0][2] * pz + t[0];
        double y = m[1][0] * px + m[1][1] * py + m[1][2] * pz + t[1];
        double z = m[2][0] * px + m[2][1] * py + m[2][2] * pz + t[2];

        return new AtomSite(atom.getElement(), atom.getCharge(), new Vector3D(
                x, y, z), atom.getOccupancy());
    }
